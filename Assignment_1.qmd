...
HTML version can be found [here]{.underline} (<https://benjamintee.github.io/CASA_ABM_Assessment/Assignment_1.html>)  
...
[*Experiment 2 (S3)*]{.underline}*:* *inheritance-rate* was varied at intervals of 0.25 between 0.00 and 1.00, with *inherit-location?* toggled on and off, holding geography constant with *num-hills = 2*. This was subsequently repeated at finer intervals of 0.05 to identify tipping points. 
...
However, enabling location inheritance generally reduces inequality compared to wealth inheritance alone. At *inheritance-rate* = 0, simply allowing offspring to start near their parent's location reduces the Gini from **0.458** to **0.396**. Offspring of successful agents are born close to productive resource hills, reducing search cost and early starvation risk. Over successive generations, agents gradually optimise their positions, compressing the gini distribution from below.

[Figure 5]{.underline} traces the mechanism in detail. Location inheritance primarily operates through improved mortality. Starvation deaths fall from **5,265** to **4,028** at *inheritance-rate* = 0 when location inheritance is added, allowing more agents to survive to old age rather than dying in resource-scarce areas. 
...
```{python}
df_S3a_agg = pd.read_csv(r"C:\Users\benja\Documents\CASA\CASA\Agent-Based Models and Spatial Systems\CASA_ABM_Assessment\data\Sugarscape 3 S3_baseline_a (aggregate)-spreadsheet.csv", skiprows=15)

df_S3b_agg = pd.read_csv(r"C:\Users\benja\Documents\CASA\CASA\Agent-Based Models and Spatial Systems\CASA_ABM_Assessment\data\Sugarscape 3 S3_baseline_b (aggregate)-spreadsheet.csv", skiprows=15)

df_S3a_agg = df_S3a_agg.iloc[:, 1:]
df_S3b_agg = df_S3b_agg.iloc[:, 1:]
...
n_runs = 10
n_rates = 21  # 0, 0.05, 0.10, ..., 1.00
n_reps = n_runs * n_rates  # 210
...
df_S3a_agg.columns = new_columns
df_S3b_agg.columns = new_columns

# Since it's a single row, reshape directly
def reshape_single_row(df, base_cols, n_reps):
    records = []
    row = df.iloc[0]
    for i in range(n_reps):
        record = {col: row[f"{col}_{i}"] for col in base_cols}
        record['repetition'] = i
        records.append(record)
    return pd.DataFrame(records)

df_S3a_long = reshape_single_row(df_S3a_agg, base_cols, n_reps)
df_S3b_long = reshape_single_row(df_S3b_agg, base_cols, n_reps)
```

```{python}
for metric, ylabel, fig_title in [
    ('mean_wealth', 'Mean Wealth', 'Figure 6: Mean Wealth by Inheritance Rate With and Without \nLocation Inheritance (t=1000, n=10)'),
    ('gini', 'Gini Coefficient', 'Figure 7: Gini by Inheritance Rate With and Without \nLocation Inheritance (t=1000, n=10)')
    
]:
    fig, ax = plt.subplots(figsize=(8, 5))

    for df, label, color in [
        (df_S3a_long, 'Without Location Inheritance', '#2ca02c'),
        (df_S3b_long, 'With Location Inheritance', '#ff7f0e')
    ]:
        grouped = df.groupby('inheritance-rate')[metric]
        mean = grouped.mean()
        std = grouped.std()
    ...
    ax.set_xlabel('inheritance-rate', fontsize=11)
    ax.set_ylabel(ylabel, fontsize=11)
    ax.set_title(fig_title, fontsize=12, fontweight='bold')
    ax.legend(fontsize=11, prop={'weight': 'bold'}, loc='upper left', frameon=False)
